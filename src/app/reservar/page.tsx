...
function computeExpectedAmountMXN(startIso: string, endIso: string) {
  ...
  for (let t = startMs; t < endMs; t += 60_000) {
    ...
    const rate = rateAtMinute(new Date(t).toISOString()); // usa la hora local de ese minuto
    total += hours * rate;
  }
  return Math.round(total); // redondeo a peso, cámbialo si quieres centavos
}
...
function priceLabelForRange(startIso: string, endIso: string) {
  ...
  // Si todo empieza y termina antes de 18 => 350
